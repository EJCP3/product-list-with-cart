...
export default function Product({ product }) {
  ...
  return (
    <div className="">
      <figure className="w-80 h-auto sm:w-60  relative mb-6">
        <img
          src={product.image.desktop}
          className={`${
            checkProductQuantity(product) > 0
              ? "border-red border-2"
              : "border-none"
          ...
          {checkProductQuantity(product) > 0 ? (
            <div className="flex bg-red py-3 px-7 border-1 rounded-4xl gap-x-1 w-40 justify-between items-center">
              <span
                ...
                className="flex justify-center items-center size-5 border-1 rounded-full border-white "
              >
                <IncrementIcon />
              </span>
            ...
            <button
              onClick={() => addToCart(product)}
              className="flex bg-white py-3 px-7 border-1 border-rosi-500 rounded-4xl gap-x-1 font-semibold text-base"
            >
              <AddCartIcon />
              Add To Cart
            </button>
